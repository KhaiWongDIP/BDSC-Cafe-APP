       ...
       #-------------------------------------------------Forgot Password Widgets--------------------------------------------
        ...
        def Orders():
            ...
            for x in range(NumOfItems):
                print(CafeOrders[x])
            OnedollarItems = sum(x.count(1) for x in CafeOrders)
            One_FivedollarItems = 1.5 * sum(x.count(1.5) for x in CafeOrders)
            TwodollarItems = 2 * sum(x.count(2) for x in CafeOrders)
            ThreedollarItems = 3 * sum(x.count(3) for x in CafeOrders)
            Three_FivedollarItems = 3.5 * sum(x.count(3.5) for x in CafeOrders)
            FourdollarItems = 4 * sum(x.count(4) for x in CafeOrders)
            Four_FivedollarItems = 4.5 * sum(x.count(4.5) for x in CafeOrders)
            CafeTotal = OnedollarItems + One_FivedollarItems + TwodollarItems + ThreedollarItems + Three_FivedollarItems + FourdollarItems + Four_FivedollarItems
            print("Total Price ${}".format(CafeTotal))   
